...
class DurationModelWrapper(PythonModel):
    ...
    def predict(self, context, model_input):
        dicts = model_input[["start_station_id", "end_station_id"]].to_dict(
            orient="records"
        )
...
def main():
    ...
    with mlflow.start_run():
        ...
        signature = infer_signature(
            input_example, wrapped_model.predict(None, input_example)
        )
        ...
        client.set_registered_model_alias(
            name="bluebikes-duration-model", alias="champion", version=latest_version
        )
